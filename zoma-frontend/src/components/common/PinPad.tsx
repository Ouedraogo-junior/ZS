    ...
    <div className="grid grid-cols-3 gap-3">
      {keys.map((key, i) => (
        <button
          ...
          className={`h-[60px] rounded-2xl text-xl font-bold transition-all select-none flex items-center justify-center ${
            key === '' ? 'invisible' :
            key === 'delete' ? 'bg-border text-muted active:bg-border/70' :
            'bg-white text-primary shadow-sm active:bg-primary active:text-white'
          } ${disabled ? 'opacity-50' : ''}`}
